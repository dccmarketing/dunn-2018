...
class Sanitize {
	...
	public function clean( $data, $type ) {
		...
		if ( empty( $type ) ) {
			...
			$check = new \WP_Error( 'forgot_type', __( 'Specify the data type to sanitize.', 'dunn' ) );

		}

		if ( is_wp_error( $check ) ) {

			wp_die( $check->get_error_message(), __( 'Forgot data type', 'dunn' ) );
		 ...
		 */
		do_action( 'dunn_pre_sanitize', $sanitized );
		 ...
		 */
		do_action( 'dunn_post_sanitize', $sanitized );
